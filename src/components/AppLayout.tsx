      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-900 text-white sticky top-0 h-screen shadow-xl border-r border-white/5">
        ...
        <div className="p-6 border-t border-white/5 bg-black/20">
          <div className="flex items-center gap-3 px-2 py-3 mb-4">
            <div className="size-10 rounded-xl bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center text-white font-black text-sm shadow-lg shadow-orange-500/20">
              {user?.email?.charAt(0).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold truncate text-slate-200">{user?.email?.split('@')[0]}</p>
              <p className="text-[10px] text-orange-500/80 uppercase font-black tracking-widest">{role}</p>
            </div>
          </div>
          <Button 
            variant="ghost" 
